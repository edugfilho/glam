...
--   --destination_table=firefox_desktop_glam_beta__clients_histogram_aggregates_v1 \
--   --reservation_id=moz-fx-bigquery-reserv-global:US.glam \
--   < firefox_desktop_glam_beta__step2_merge.sql
...
WITH extracted_accumulated AS (
  SELECT
    *
  FROM
    `moz-fx-glam-prod.glam_etl.firefox_desktop_glam_beta__clients_histogram_aggregates_v1`
  WHERE
    sample_id >= 0
    AND sample_id <= 99
),
filtered_accumulated AS (
  ...
  LEFT JOIN
    `moz-fx-glam-prod.glam_etl.firefox_desktop_glam_beta__latest_versions_v1`
    USING (channel)
  WHERE
...
transformed_daily AS (
  SELECT
    *
  FROM
    `moz-fx-glam-prod.glam_etl.firefox_desktop_glam_beta__clients_histogram_aggregates_new_v1`
  WHERE
    sample_id >= 0
    AND sample_id <= 99
)
