-- STEP 2: merge the metrics delta in _new_v1 into _v1.
-- The accumulated-side build window uses CURRENT_DATE(), not @submission_date, so
-- current builds are not dropped. This query takes no parameters.
--
-- bq query --use_legacy_sql=false --replace \
--   --project_id=moz-fx-glam-prod --dataset_id=moz-fx-glam-prod:glam_etl \
--   --destination_table=firefox_desktop_glam_nightly__clients_histogram_aggregates_v1 \
--   --reservation_id=moz-fx-bigquery-reserv-global:US.glam \
--   < firefox_desktop_glam_nightly__step2_merge.sql
...
WITH extracted_accumulated AS (
  SELECT
    *
  FROM
    `moz-fx-glam-prod.glam_etl.firefox_desktop_glam_nightly__clients_histogram_aggregates_v1`
  WHERE
    sample_id >= 0
    AND sample_id <= 99
),
filtered_accumulated AS (
  ...
  LEFT JOIN
    `moz-fx-glam-prod.glam_etl.firefox_desktop_glam_nightly__latest_versions_v1`
    USING (channel)
  WHERE
...
transformed_daily AS (
  SELECT
    *
  FROM
    `moz-fx-glam-prod.glam_etl.firefox_desktop_glam_nightly__clients_histogram_aggregates_new_v1`
  WHERE
    sample_id >= 0
    AND sample_id <= 99
)
